// Last updated: 6/29/2026, 10:28:27 PM
1class Solution {
2public:
...
14                if(grid[i][j] == 1){
15                    int perimeter = bfs(i, j, grid); // we are guaranteed we have only 1 island 
16                    return perimeter; 
17                }
18            }
19        }
...
22    }
23
24    int bfs(int x, int y, vector<vector<int>> &grid){
25        int dx[4] = {0, 0, -1, 1};
26        int dy[4] = {1, -1, 0, 0};
...
40                if(newX >= 0 && newX < rows && newY >= 0 && newY < cols && grid[newX][newY] == 1){
41                    grid[newX][newY] = -1;
42                    q.push({newX, newY});
